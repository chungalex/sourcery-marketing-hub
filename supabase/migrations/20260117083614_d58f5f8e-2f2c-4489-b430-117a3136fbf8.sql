...
-- Replace transition_order_status function with all fixes
...
BEGIN
  ...
  v_valid_transitions := CASE v_old_status
    ...
    WHEN 'disputed' THEN ARRAY['ready_to_ship','shipped','closed']::public.order_status[]  -- FIX #4: sensible dispute resolutions
  ...
  -- FIX #5: Prevent cancellation after payment released
  ...
  IF p_new_status = 'ready_to_ship' THEN
    IF NOT EXISTS (
      ...
      RETURN jsonb_build_object(
        'success', false,
        'error', 'Cannot mark ready to ship: at least one milestone payment must be released'  -- FIX #1: clearer message
      );
    END IF;
  END IF;
  ...
  v_allowed_actor_types := CASE
    ...
    WHEN (v_old_status = 'qc_uploaded' AND p_new_status IN ('qc_pass','qc_fail')) THEN ARRAY['qc','admin']  -- FIX #3: QC can set result
    WHEN (v_old_status = 'qc_pass' AND p_new_status = 'ready_to_ship') THEN ARRAY['factory','admin']
    WHEN (v_old_status = 'ready_to_ship' AND p_new_status = 'shipped') THEN ARRAY['factory','admin']  -- FIX #2: factory marks shipped
    ...
    WHEN (v_old_status = 'disputed' AND p_new_status IN ('ready_to_ship','shipped','closed')) THEN ARRAY['admin']  -- FIX #4: updated resolutions
